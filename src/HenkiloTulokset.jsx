...
export default function HenkiloTulokset({ rawCsv }) {
  const [valittuAmpuja, setValittuAmpuja] = useState(null);
  ...
  const siivoa = (s) => (!s ? "" : s.replace(/^"|"$/g, '').trim());

  const rivit = rawCsv.split(/\r?\n/).filter(r => r.trim() !== "");
  if (rivit.length < 2) return <div style={tyylit.Viesti}>Ei tulosdataa.</div>;

  // 1. ETSITÄÄN OTSIKOIDEN PAIKAT
  const otsikot = rivit[0].split(',').map(siivoa).map(o => o.toUpperCase());
  ...
  for (let i = 1; i < rivit.length; i++) {
    ...
    ampujat.push(ampuja);
    if (ampuja.sarja) loydetytSarjat.add(ampuja.sarja);
  }

  // 3. SUODATUS JA JÄRJESTYS
  let naytettavatAmpujat = [];

  if (sarjaSuodatin === 'KAIKKI') {
    naytettavatAmpujat = ampujat.map(a => ({
      ...a,
      laskettuSija: a.alkuperainenSija
    }));
  } else {
    const sarjanVaki = ampujat.filter(a => a.sarja.toUpperCase() === sarjaSuodatin.toUpperCase());
    ...
    naytettavatAmpujat = sarjanVaki.map((ampuja, index, array) => {
      let sija = index + 1;
      if (index > 0) {
        const edellinen = array[index - 1];
        if (edellinen.tulos === ampuja.tulos && edellinen.ratko === ampuja.ratko) {
          sija = edellinen.laskettuSija || index; 
        }
      }
  ...
  return (
    <div style={tyylit.Alue}>
      ...
      <div style={tyylit.SuodatinPalkki}>
        <button onClick={() => { setSarjaSuodatin('KAIKKI'); setValittuAmpuja(null); }} style={sarjaSuodatin === 'KAIKKI' ? tyylit.SuodatinNappiAktiivinen : tyylit.SuodatinNappi}>KAIKKI</button>
        {Array.from(loydetytSarjat).sort().map(sarja => (
          <button key={sarja} onClick={() => { setSarjaSuodatin(sarja); setValittuAmpuja(null); }} style={sarjaSuodatin === sarja ? tyylit.SuodatinNappiAktiivinen : tyylit.SuodatinNappi}>{sarja}</button>
        ))}
      </div>
        ...
        <table style={tyylit.Taulukko}>
          ...
          <tbody>
            {naytettavatAmpujat.map((ampuja, index) => {
              ...
              const onAuki = valittuAmpuja === index;
              ...
              return (
                <React.Fragment key={index}>
                  <tr 
                    ...
                    onClick={() => setValittuAmpuja(onAuki ? null : index)}
                  >
                  ...
                  {onAuki && ampuja.sarjat.length > 0 && (
                    <tr>
                      <td colSpan={7} style={tyylit.LaajennusSolu}>
                        <div style={tyylit.SarjaRuudukko}>
                          {ampuja.sarjat.map((s, sIdx) => (
                            <div key={sIdx} style={tyylit.SarjaSolu}>
